function [AP_sizes_list, AP_actual_sizes_table, potential_AP_number]=CurrentStepsFunctionE(data, filename, output_folder, m, show_figures, location, name)
...
filter = 10000;
step_size = 20;
current_injection = 0:step_size:(step_number(sweeps)-2)*step_size;
baseline_position = 1:2200;
data_position = duration_start:duration_end;
AP_size_limit = 15;
% Magic end
...
for j=1:sweeps
    ...
    fire = 0;
    if j==1
        baseline_mean=mean(data(baseline_position));
        step_mean=mean(data(data_position));
        input_resistance=(baseline_mean-step_mean)/step_size*1000;     
    end
    ...
    for i = duration
        ...
        if mavgcount > window
           mavg = mavg - sweep_data(i-1-window);
           mavgcount = mavgcount - 1;
        end
        ...
        if sweep_data(i) > mavg / mavgcount || i == duration_end %increasing
            if fire == 0
                ...
                if potential_AP_number > 0
                    ...
                    AP_min_last = AP_min;
                    AP_last_real = 0;
                    if AP_size > AP_size_limit && (AP_number == 0 || AP_size > 0.5 * AP_sizes(AP_number) || AP_size > AP_size_limit + 5)
...
title_pos = strcat(ExcelCol(m+1), '4');
freq_pos = strcat(ExcelCol(m+1), '5');
input_pos = strcat(ExcelCol(m+1), '22');
APs_pos = strcat(ExcelCol(m+1), '24');
...
excel_name = sprintf('%s\\CS freq APs summary_%s.xlsx', location, date); %it tells the full path of the file
xlswrite(excel_name, {'Genotype'}, 1, 'A2');
xlswrite(excel_name, {'Capacitance'}, 1, 'A3');
xlswrite(excel_name, current_injection', 1, 'A5');
xlswrite(excel_name, {filename}, 1, title_pos{1});
xlswrite(excel_name, average_frequencies(2:16), 1, freq_pos{1});
xlswrite(excel_name, current_injection', 1, 'A24');
xlswrite(excel_name, frequency_list(2:16), 1, APs_pos{1});
xlswrite(excel_name, {'Input resistance'}, 1, 'A22');
