function [AP_sizes_list, AP_actual_sizes_table, potential_AP_number]=CurrentStepsFunctionZ(data, filename, output_folder, m, show_figures, location, name)
...
for j=1:sweeps
    ...
    if j==1
        baseline_mean=mean(data(1:4500));
        step_mean=mean(data(4800:14500));
        input_resistance=(baseline_mean-step_mean)/20*1000;     
    end
...
input_pos = strcat(ExcelCol(m+1), '37');
APs_pos = strcat(ExcelCol(m+1), '21');
...
xlswrite(excel_name, current_injection', 1, 'A21');
xlswrite(excel_name, frequency_list(2:16), 1, APs_pos{1});
xlswrite(excel_name, {'Input resistance'}, 1, 'A37');
